...
#define MSG_TYPE_FORMAT_SDCARD      0x29
#define MSG_TYPE_PLAYALARM          0x30
#define MSG_TYPE_GET_ALERM_LOG      0x31
...
typedef struct tag_AV_HEAD
{
    ...
    unsigned int    len;   //  ˝æ›≥§∂»:        ≤ªº∆À„Õ∑
    unsigned int    cmd;        // √¸¡Ó¬Î:         ∂‘”¶tutkµƒIOCtrlType[ENUM_AVIOCTRL_MSGTYPE]
    unsigned char   sckt_ch;      // Ã◊∞¥◊÷Õ®µ¿:     T_SCKT_CH_TYPE,À‰»ªÀ˚”–channelø…”√,◊ˆ‘⁄’‚¿ÔŒ™¡À“‘∫Û∑Ω±„œÚ∆‰À¸p2p¿©’π
    unsigned char   reserv[3];  // ±£¡Ù
    
    unsigned short codec_id;
    unsigned char type;            // Combined with IPC_FRAME_xxx.
    unsigned char cam_index;        // 0 - n
    unsigned char onlineNum;        // number of client connected this device
    unsigned char reserve1[3];
    unsigned int reserve2;  // utc
    unsigned int sectime; // Timestamp of the frame, in milliseconds
...
typedef struct tag_STRUCT_JPUSH_PARAM{
    char appkey[64];
    char master[64];
    char alias[64];
    int type;
}STRUCT_JPUSH_PARAM,*PSTRUCT_JPUSH_PARAM;


typedef struct tag_STRUCT_JPUSH_PARAM_Test{
    char appkey[1024][128];
    char master[64];
    char alias[64];
    int type;
...
#ifdef __cplusplus
    tag_STRUCT_JPUSH_PARAM_Test()
    {
        
        
        memset(this,0,sizeof(tag_STRUCT_JPUSH_PARAM_Test));
        //或者是下面的格式
...
}STRUCT_JPUSH_PARAM_Test,*PSTRUCT_JPUSH_PARAM_Test;


typedef struct tag_FILE_BY_TIME{
    char filename[128];
}FILE_BY_TIME,*PFILE_BY_TIME;

typedef struct tag_STRU_SDCARD_RECORD_FILE_BY_TIME
{
    int 			nFileCount;
    ///   char 			szFileName[672][128];	//for 7 day
     //FILE_BY_TIME	file[10];
     //char 			szFileName[67];
    unsigned int 	starttime[672];
    unsigned int	endtime[672];
    int 			nFileType[672];
}STRU_SDCARD_RECORD_FILE_BY_TIME, *PSTRU_SDCARD_RECORD_FILE_BY_TIME;
#endif
