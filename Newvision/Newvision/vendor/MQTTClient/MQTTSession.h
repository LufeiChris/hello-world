...
/** connects to the specified MQTT server
 ...
 if (error) {
 MYLog(@"Error Connect %@", error.localizedDescription);
 } else {
 MYLog(@"Connected sessionPresent:%d", sessionPresent);
 }
 }];
 @endcode
...
/** subscribes to a topic at a specific QoS level
 ...
 [session subscribeToTopic:@"example/#" atLevel:2 subscribeHandler:^(NSError *error, NSArray<NSNumber *> *gQoss){
    if (error) {
        MYLog(@"Subscription failed %@", error.localizedDescription);
    } else {
        MYLog(@"Subscription sucessfull! Granted Qos: %@", gQoss);
    }
 }];
...
/** closes an MQTTSession gracefully
 ...
 [session closeWithDisconnectHandler^(NSError *error) {
    if (error) {
        MYLog(@"Error Disconnect %@", error.localizedDescription);
    }
    MYLog(@"Session closed");
 }];
