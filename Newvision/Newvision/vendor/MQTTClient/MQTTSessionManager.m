...
//解除订阅
- (void)unSubscriptions:(NSDictionary<NSString *, NSNumber *> *)newSubscriptions{
            ...
            if (![currentSubscriptions objectForKey:topicFilter]) {
                ...
                [self.session subscribeToTopic:topicFilter atLevel:qos subscribeHandler:^(NSError *error, NSArray<NSNumber *> *gQoss) {
                    if (!error) {
                        ...
                        NSLog(@"订阅成功：%@",topicFilter);
                        
                        //发送登录注册主题订阅成功通知
                        if ([topicFilter rangeOfString:@"reglogin"].location != NSNotFound) {
                            
                            [[NSNotificationCenter defaultCenter] postNotificationName:kRegloginSubscriptionsSuccess object:nil];
                            
                        }
                        //发送家庭主题订阅成功通知
                        if ([topicFilter rangeOfString:@"family"].location != NSNotFound) {
                            [[NSNotificationCenter defaultCenter] postNotificationName:kFamilySubscriptionsSuccess object:nil];
                        }
                    }
                }];
            }
